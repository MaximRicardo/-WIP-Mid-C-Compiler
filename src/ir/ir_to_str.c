...
static void instr_to_str(struct DynamicStr *output, const struct IRInstr *instr)
{
    u32 i;
...
static char *func_args_to_str(const struct IRFunc *func)
{
    ...
    for (i = 0; i < func->args.size; i++) {
        ...
        DynamicStr_append_printf(&final_str, "%s %%%s", type_str,
                                 func->args.elems[i].name);
...
static void func_to_str(struct DynamicStr *output, const struct IRFunc *func)
{
    ...
    DynamicStr_append_printf(output, "define %s %s @%s(%s)", func_mods_str,
                                  ...
                                  const struct IRArrayLitList *list)
{
    ...
    for (i = 0; i < list->size; i++) {
        ...
        struct IRDataType type = IRDataType_create(false, lit->elem_width, 0);
