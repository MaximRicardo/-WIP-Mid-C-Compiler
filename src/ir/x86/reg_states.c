...
static const char *reg_states_merge_vregs(const char *self_vreg,
                                          const char *other_vreg,
                                          const struct IRRegLTList *vreg_lts)
{
    if (!self_vreg) {
        return other_vreg;
    } else if (other_vreg) {
        u32 self_vreg_i = IRRegLTList_find_reg(vreg_lts, self_vreg);
        u32 other_vreg_i = IRRegLTList_find_reg(vreg_lts, other_vreg);
                     ...
                     const struct IRRegLTList *vreg_lts)
{
    ...
    for (i = 0; i < m_n_usable_pregs; i++) {
        self->preg_vals[i].virt_reg =
            reg_states_merge_vregs(self->preg_vals[i].virt_reg,
                                   other->preg_vals[i].virt_reg, vreg_lts);
    }
}
