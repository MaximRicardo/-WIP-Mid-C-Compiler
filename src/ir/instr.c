...
bool IRInstrType_has_dest_reg(enum IRInstrType type)
{
    return IRInstrType_is_bin_op(type) || type == IRInstr_LOAD ||
           type == IRInstr_ALLOCA || type == IRInstr_PHI || type == IRInstr_MOV;
}
                                              ...
                                              const char *reg)
{
    ...
    x.type =
        expr->expr_type == ExprType_INT_LIT ? IRInstrArg_IMM32 : IRInstrArg_REG;

    x.data_type = IRDataType_create_from_prim_type(
        expr->prim_type, expr->type_idx, expr->lvls_of_indir, structs);

    x.value = x.type == IRInstrArg_REG ? IRInstrArgValue_reg_name(reg)
              : x.data_type.is_signed
                  ? IRInstrArgValue_imm_i32(expr->int_value)
...
struct IRInstr IRInstr_create(enum IRInstrType type, struct IRInstrArgList args)
{
    struct IRInstr instr;
    instr.type = type;
    instr.args = args;
                                  ...
                                  struct IRInstrArg arg)
{
    ...
    IRInstrArgList_push_back(&instr.args,
                             IRInstrArg_create(IRInstrArg_REG, dest_d_type,
                                               IRInstrArgValue_reg_name(dest)));
...
struct IRInstr IRInstr_create_str_instr(enum IRInstrType type, const char *dest)
{
    struct IRInstr instr = IRInstr_init();
    instr.type = type;
    IRInstrArgList_push_back(
        &instr.args, IRInstrArg_create(IRInstrArg_STR, IRDataType_init(),
                                       IRInstrArgValue_generic_str(dest)));
    return instr;
}

struct IRInstr IRInstr_create_cond_jmp_instr(enum IRInstrType type,
                                             struct IRInstrArg cmp_lhs,
                                             struct IRInstrArg cmp_rhs,
                                             const char *true_dest,
                                             const char *false_dest)
{
    struct IRInstr instr = IRInstr_init();
    instr.type = type;
    ...
    IRInstrArgList_push_back(
        &instr.args,
        IRInstrArg_create(IRInstrArg_STR, IRDataType_init(),
                                        ...
                                        struct IRDataType d_type)
{
    ...
    IRInstrArgList_push_back(
        &instr.args, IRInstrArg_create(IRInstrArg_REG, d_type,
                                       IRInstrArgValue_reg_name(reg_name)));

    return instr;
}

struct IRInstr IRInstr_create_alloca(const char *dest_vreg,
                                     struct IRDataType d_type, u32 size,
                                     u32 alignment)
{
    ...
    IRInstrArgList_push_back(&instr.args,
                             IRInstrArg_create(IRInstrArg_IMM32, u32_d_type,
                                               IRInstrArgValue_imm_u32(size)));

    IRInstrArgList_push_back(
        &instr.args, IRInstrArg_create(IRInstrArg_IMM32, u32_d_type,
                                       IRInstrArgValue_imm_u32(alignment)));
                                   ...
                                   const struct TranslUnit *tu)
{
    ...
    IRInstrArgList_push_back(
        &instr.args, IRInstrArg_create(IRInstrArg_REG, dest_d_type,
                                       IRInstrArgValue_reg_name(dest_vreg)));
    ...
    for (i = 0; i < arg_vregs->size; i++) {
        ...
        struct IRDataType d_type = IRDataType_create_from_prim_type(
            arg_expr->prim_type, arg_expr->type_idx, arg_expr->lvls_of_indir,
            tu->structs);

        IRInstrArgList_push_back(
            &instr.args,
            IRInstrArg_create(IRInstrArg_REG, d_type,
                       ...
                       bool count_assignment)
{
    ...
    u32 start_idx =
        count_assignment && IRInstrType_has_dest_reg(self->type) ? 1 : 0;
