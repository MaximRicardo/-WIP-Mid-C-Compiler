                                  ...
                                  struct IRRegLTList *lts)
{
    ...
    for (i = 0; i < instr->args.size; i++) {
        ...
        if (reg_idx != m_u32_max) {
            ...
            IRRegLTList_push_back(
                lts, IRRegLT_create(reg_name, *n_instrs, *n_instrs));
        }
    }
...
static bool block_exits_use_vreg(const struct IRBasicBlock *block,
                                 ...
                                 struct U32List *ignore_list, const char *vreg)
{
    u32 i;
                              ...
                              struct IRRegLTList *lts)
{
    ...
    for (i = 0; i < lts->size; i++) {
        extend_lifetime_via_exits(block, cur_func, &lts->elems[i],
                                  *n_instrs - 1);
    }
}
