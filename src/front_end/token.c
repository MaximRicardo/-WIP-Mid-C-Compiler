...
struct Token Token_create(unsigned line_num, unsigned column_num,
                          ...
                          const char *file_path, enum TokenType type)
{
    struct Token token;
...
{
    struct Token token =
        Token_create(line_num, column_num, src_start, src_len, file_path, type);
    token.value = value;
    return token;
}
...
{
    return type > TokenType_UNARY_OPS_START && type < TokenType_UNARY_OPS_END;
}

bool Token_is_bin_operator(enum TokenType type)
{
    return type > TokenType_BINARY_OPS_START && type < TokenType_BINARY_OPS_END;
}
