...
static struct BlockNode *parse(const struct Lexer *lexer,
                               struct FuncDeclNode *parent_func, u32 bp, u32 sp,
                               u32 block_start_idx, u32 *end_idx,
                               unsigned n_blocks_deep, bool *missing_r_curly,
...
{
    return m_min(
        skip_to_token_type_arr(start_idx, tokens, stop_types, n_stop_types),
        tokens.size - 1);
}

static void check_if_missing_r_curly(const struct Lexer *lexer,
                                     u32 block_start_idx, u32 block_end_idx,
                                     bool check_if_reached_end,
                                     bool *missing_r_curly)
{
    if (lexer->token_tbl.elems[block_end_idx].type != TokenType_R_CURLY ||
        (check_if_reached_end && block_end_idx + 1 == lexer->token_tbl.size)) {
        if (missing_r_curly)
            *missing_r_curly = true;
        ErrMsg_print(
            ErrMsg_on, &Parser_error_occurred,
...
{
    struct Expr *expr =
        SY_shunting_yard(&lexer->token_tbl, start_idx, NULL, 0, sy_end_idx,
...
static u32 get_array_len(const struct Lexer *lexer, u32 l_subscr_idx,
                         u32 *r_subscr_idx, const char *array_name,
                         bool allow_no_len, bool *len_was_given)
{
    enum TokenType stop_types[] = {TokenType_R_ARR_SUBSCR};
    u32 array_len;

    struct Expr *arr_len_expr = SY_shunting_yard(
        &lexer->token_tbl, l_subscr_idx + 1, stop_types,
        sizeof(stop_types) / sizeof(stop_types[0]), r_subscr_idx, &vars,
        &structs, 0, false, &typedefs, true);
...
static u32 get_array_len_token(const struct Lexer *lexer, u32 l_subscr_idx,
                               u32 *r_subscr_idx, u32 ident_idx,
                               bool allow_no_len, bool *len_was_given)
{
...
static struct Expr *var_decl_value(const struct Lexer *lexer, u32 ident_idx,
                                   u32 equal_sign_idx, u32 *semicolon_idx,
                                   u32 bp)
{
    struct Expr *expr = NULL;

    if (equal_sign_idx + 1 >= lexer->token_tbl.size ||
        lexer->token_tbl.elems[equal_sign_idx].type == TokenType_SEMICOLON) {
        if (semicolon_idx)
    ...
    expr = SY_shunting_yard(&lexer->token_tbl, equal_sign_idx + 1, NULL, 0,
                            semicolon_idx, &vars, &structs, bp, true, &typedefs,
                            true);
...
static bool verify_valid_type(const struct Lexer *lexer,
                              enum PrimitiveType var_type, u32 lvls_of_indir,
                              u32 ident_idx, u32 v_decl_idx)
{
    if (var_type == PrimType_VOID && lvls_of_indir == 0) {
        ...
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                     ...
                     " column %u.\n",
                     var_name, lexer->token_tbl.elems[v_decl_idx].line_num,
                     lexer->token_tbl.elems[v_decl_idx].column_num);
...
static bool get_var_ident(const struct Lexer *lexer, u32 v_decl_idx,
                          enum PrimitiveType *var_type, u32 *var_type_idx,
                          u32 *lvls_of_indir, struct TypeModifiers *mods,
                          ...
                          const struct StructList *structs, u32 *ident_idx)
{
    *ident_idx = TypeSpec_read(&lexer->token_tbl, v_decl_idx, var_type,
                               var_type_idx, lvls_of_indir, mods, typedefs,
...
static u32 get_var_size(enum PrimitiveType var_type, u32 var_indir,
                        bool is_array, bool len_defined, u32 *array_len,
                        bool is_func_param, struct Expr *init)
{
    u32 var_size = 0;
...
static bool verify_var_init(const struct Lexer *lexer, bool is_array,
                            bool len_defined, const struct Expr *init,
                            u32 ident_idx)
{
    ...
    if (is_array && init && init->expr_type != ExprType_ARRAY_LIT) {
        ...
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred, ident->file_path,
                     ...
                     " line %u, column %u\n",
                     var_name, ident->line_num, ident->column_num);
    ...
    if (!is_func_param) {
        ...
        *n_func_param_bytes = round_up(*n_func_param_bytes, var_size);
        decl->bp_offset = m_TypeSize_callee_saved_regs_stack_size +
                          m_TypeSize_return_address_size +
...
parse_var_declarator(const struct Lexer *lexer, u32 ident_idx, u32 *end_idx,
                     u32 bp, u32 *sp, bool is_func_param,
                     u32 *n_func_param_bytes, enum PrimitiveType var_type,
                     u32 var_indir, u32 *var_size, void *par_var_parent)
{
    struct Expr *expr = NULL;
    ...
    if (is_array) {
        array_len = get_array_len_token(lexer, ident_idx + 1, end_idx,
                                        ident_idx, true, &len_defined);
        ++*end_idx;
    ...
    *var_size = get_var_size(var_type, var_indir, is_array, len_defined,
    ...
    decl =
        Declarator_create(expr, Token_src(&lexer->token_tbl.elems[ident_idx]),
                          var_indir, is_array, array_len, 0);
                                ...
                                u32 v_decl_idx, bool is_func_param)
{
    ...
    bool f_param_end =
        end_tok->type == TokenType_COMMA || end_tok->type == TokenType_R_PAREN;

    bool missing = !(end_tok->type == TokenType_SEMICOLON ||
...
static struct VarDeclNode *parse_var_decl(const struct Lexer *lexer,
                                          u32 v_decl_idx, u32 *end_idx, u32 bp,
                                          u32 *sp, bool is_func_param,
                                          ...
                                          void *par_var_parent)
{
    ...
    if (!get_var_ident(lexer, v_decl_idx, &var_type, &var_type_idx, &var_indir,
                       &mods, &typedefs, &structs, &ident_idx)) {
    ...
    decl = parse_var_declarator(lexer, ident_idx, end_idx, bp, sp,
                                is_func_param, n_func_param_bytes, var_type,
                                var_indir, &var_size, par_var_parent);
    ...
    *var_decl = VarDeclNode_init();
    ...
    ParVarList_push_back(
        &vars,
        ParserVar_create(lexer->token_tbl.elems[v_decl_idx].line_num,
                         lexer->token_tbl.elems[v_decl_idx].column_num,
                         Token_src(&lexer->token_tbl.elems[ident_idx]),
                         var_indir, mods, var_type, var_type_idx, decl.is_array,
                         decl.array_len, decl.bp_offset + bp, NULL, false,
                                  ...
                                  u32 f_ident_tok_idx)
{
    ...
    not_matching |= !TypeModifiers_equal(
        &vars.elems[prev_func_decl_var_idx].mods, &func->ret_type_mods);
    not_matching |=
        vars.elems[prev_func_decl_var_idx].args->size != func->args.size;
    not_matching |=
        vars.elems[prev_func_decl_var_idx].variadic_args != func->variadic_args;
    not_matching |=
        vars.elems[prev_func_decl_var_idx].void_args != func->void_args;
                                  ...
                                  u32 param_start_idx, const char *param_name)
{
    ...
    if (end->type == TokenType_R_PAREN) {
        ...
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred, start->file_path,
                               ...
                               struct VarDeclPtrList *args)
{
    ...
    if (!verify_param_type_spec(lexer, param_start_idx)) {
        enum TokenType stop_types[] = {TokenType_R_PAREN, TokenType_L_CURLY};

        *end_idx = skip_to_token_type_alt_arr(
                       param_start_idx, lexer->token_tbl, stop_types,
    ...
    arg = parse_var_decl(lexer, param_start_idx, end_idx, bp, NULL, true,
                         n_func_param_bytes, func_node);
...
static u32 parse_func_params_loop(const struct Lexer *lexer,
                                  u32 params_start_idx, bool *variadic_args,
                                  u32 bp, struct VarDeclPtrList *args,
                                  struct FuncDeclNode *func_node)
{
    ...
    *variadic_args = false;
    ...
    while (end_idx < lexer->token_tbl.size &&
        ...
        last = parse_a_func_param(lexer, start_idx, &end_idx, bp, &param_bytes,
                                  func_node, args);
...
static u32 parse_func_params(const struct Lexer *lexer, u32 params_start_idx,
                             struct VarDeclPtrList *args, bool *void_args,
                             u32 bp, struct FuncDeclNode *func_node,
                             bool *variadic_args)
{
    *variadic_args = false;
                            ...
                            u32 f_decl_idx, u32 *end_idx, u32 bp)
{
    ...
    f_ident_idx =
        TypeSpec_read(&lexer->token_tbl, f_decl_idx, &func_type, &func_type_idx,
                      &func_lvls_of_indir, &func_type_mods, &typedefs, &structs,
    ...
    if (prev_func_decl_var_idx == m_u32_max) {
        ...
        ParVarList_push_back(
            &vars, ParserVar_create(
                       ...
                       func_lvls_of_indir, func_type_mods, func_type, 0, false,
                       0, 0, &func->args, false, false, false, false, block));
        ++old_vars_size;
    }
    ...
    if (args_end_idx == m_u32_max) {
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                     ...
                     " line %u\n",
                     func_name, lexer->token_tbl.elems[f_decl_idx].line_num);
        m_free(func_name);
        m_free(func);
    ...
    *func =
        FuncDeclNode_create(args, variadic_args, void_args, func_lvls_of_indir,
                            func_type_mods, func_type, func_type_idx, NULL,
                            Token_src(&lexer->token_tbl.elems[f_ident_idx]));

    if (prev_func_decl_var_idx != m_u32_max &&
        !func_prototypes_match(lexer, prev_func_decl_var_idx, func,
                               f_ident_idx)) {
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                     lexer->token_tbl.elems[f_decl_idx].file_path,
    ...
    ASTNodeList_push_back(
        &block->nodes,
        ASTNode_create(lexer->token_tbl.elems[f_decl_idx].line_num,
                       lexer->token_tbl.elems[f_decl_idx].column_num,
                       ASTType_FUNC, func));
...
static bool verify_ret_type(const struct Lexer *lexer,
                            const struct RetNode *node,
                            const struct FuncDeclNode *parent, u32 ret_tok_idx)
{
...
static void get_ret_stmt_value(const struct Lexer *lexer,
                               struct RetNode *ret_node, u32 ret_tok_idx,
                               u32 *end_idx, u32 bp)
{
    ret_node->value =
        SY_shunting_yard(&lexer->token_tbl, ret_tok_idx + 1, NULL, 0, end_idx,
                         &vars, &structs, bp, false, &typedefs, true);
                          ...
                          u32 n_stack_frames_deep)
{
    ...
    *node = RetNode_init();
    ...
    if (ret_tok_idx + 1 < lexer->token_tbl.size &&
        lexer->token_tbl.elems[ret_tok_idx + 1].type == TokenType_SEMICOLON) {
    ...
    ASTNodeList_push_back(
        &block->nodes,
        ASTNode_create(lexer->token_tbl.elems[ret_tok_idx].line_num,
...
static bool verify_stmt_left_paren(const struct Lexer *lexer, u32 stmt_tok_idx,
                                   enum ASTNodeType type)
{
    ...
    if (!tok_exists || tok->type != TokenType_L_PAREN) {
        ...
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                     ...
                     lexer->token_tbl.elems[stmt_tok_idx].line_num);
...
static bool verify_stmt_right_paren(const struct Lexer *lexer, u32 stmt_tok_idx,
                                    u32 r_paren_tok_idx, enum ASTNodeType type)
{
    ...
    if (!tok_exists || tok->type != TokenType_R_PAREN) {
        ...
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                     ...
                     " column %u\n",
                     str, lexer->token_tbl.elems[stmt_tok_idx + 1].line_num,
...
static struct Expr *get_cond_stmt_expr(const struct Lexer *lexer,
                                       u32 stmt_tok_idx, u32 *r_paren_idx,
                                       u32 bp)
{
    enum TokenType sy_stop_types[] = {TokenType_R_PAREN};
    return SY_shunting_yard(&lexer->token_tbl, stmt_tok_idx + 2, sy_stop_types,
                            sizeof(sy_stop_types) / sizeof(sy_stop_types[0]),
                            r_paren_idx, &vars, &structs, bp, false, &typedefs,
                            true);
}
...
static void parse_stmt_bodies(const struct Lexer *lexer, void *stmt_node,
                              u32 r_paren_idx, u32 *end_idx, u32 n_blocks_deep,
                              struct FuncDeclNode *parent_func, u32 sp, u32 bp,
                              enum ASTNodeType stmt_type)
{
    struct BlockNode *body = NULL;
    struct BlockNode *else_body = NULL;
    bool body_in_block =
        lexer->token_tbl.elems[r_paren_idx + 1].type == TokenType_L_CURLY;
    ...
    if (stmt_type == ASTType_IF_STMT && *end_idx + 1 < lexer->token_tbl.size &&
        ...
        else_body = parse(
            ...
            else_body_start_idx, end_idx, n_blocks_deep + else_body_in_block,
                         ...
                         struct FuncDeclNode *parent_func)
{
    ...
    *if_node = IfNode_init();
    ...
    if (!verify_stmt_right_paren(lexer, if_idx, r_paren_idx, ASTType_IF_STMT)) {
    ...
    if (!stmt_has_body(lexer, r_paren_idx)) {
        ...
        ASTNodeList_push_back(
            &block->nodes,
            ASTNode_create(lexer->token_tbl.elems[if_idx].line_num,
    ...
    ASTNodeList_push_back(
        &block->nodes, ASTNode_create(lexer->token_tbl.elems[if_idx].line_num,
                                      lexer->token_tbl.elems[if_idx].column_num,
                                      ASTType_IF_STMT, if_node));
                            ...
                            struct FuncDeclNode *parent_func)
{
    ...
    *while_node = WhileNode_init();
    ...
    if (!stmt_has_body(lexer, r_paren_idx)) {
        ASTNodeList_push_back(
            &block->nodes,
            ASTNode_create(lexer->token_tbl.elems[while_idx].line_num,
...
{
    node->init =
        SY_shunting_yard(&lexer->token_tbl, stmt_idx + 2, NULL, 0, r_paren_idx,
                         &vars, &structs, bp, false, &typedefs, true);
    ...
    node->condition = SY_shunting_yard(&lexer->token_tbl, *r_paren_idx + 1,
                                       NULL, 0, r_paren_idx, &vars, &structs,
                                       bp, false, &typedefs, true);
    if (*r_paren_idx >= lexer->token_tbl.size) {
        ErrMsg_print(
            ErrMsg_on, &Parser_error_occurred,
            lexer->token_tbl.elems[stmt_idx].file_path,
...

    {
        ...
        node->inc = SY_shunting_yard(
            &lexer->token_tbl, *r_paren_idx + 1, stop_types,
            sizeof(stop_types) / sizeof(stop_types[0]), r_paren_idx, &vars,
            &structs, bp, false, &typedefs, true);
    }
    if (*r_paren_idx >= lexer->token_tbl.size) {
        ErrMsg_print(
            ErrMsg_on, &Parser_error_occurred,
                          ...
                          struct FuncDeclNode *parent_func)
{
    ...
    *for_node = ForNode_init();
    ...
    if (!stmt_has_body(lexer, r_paren_idx)) {
        ASTNodeList_push_back(
            &block->nodes,
            ASTNode_create(lexer->token_tbl.elems[for_idx].line_num,
...
static u32 parse_typedef(const struct Lexer *lexer, u32 typedef_idx)
{
    ...
    unsigned type_name_idx =
        TypeSpec_read(&lexer->token_tbl, conv_type_spec_idx, &conv_type,
                      &conv_type_idx, &conv_lvls_of_indir, &conv_mods,
    ...
    if (Ident_type_spec(type_name, &typedefs) != PrimType_INVALID &&
        ...
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                     ...
                     " column %u.\n",
                     type_name, lexer->token_tbl.elems[type_name_idx].line_num,
                     lexer->token_tbl.elems[type_name_idx].column_num);
        m_free(type_name);
    } else {
        TypedefList_push_back(&typedefs,
                              Typedef_create(type_name, conv_type,
                             ...
                             struct Struct *new_struct)
{
    ...
    while (cur_field_idx < lexer->token_tbl.size &&
        ...
        field_name_idx = TypeSpec_read(
            &lexer->token_tbl, cur_field_idx, &new_field.type,
        ...
        if (field_name_idx == cur_field_idx) {
            ...
            cur_field_idx =
                skip_to_token_type_alt(cur_field_idx, lexer->token_tbl,
                                       TokenType_SEMICOLON) +
                1;
            continue;
        }
        ...
        if (field_name_idx >= lexer->token_tbl.size ||
            lexer->token_tbl.elems[field_name_idx].type != TokenType_IDENT) {
            ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
            ...
            new_field.name = Token_src(&lexer->token_tbl.elems[field_name_idx]);
        ...
        if (field_end_idx < lexer->token_tbl.size &&
            ...
            if (lexer->token_tbl.elems[l_subscr_idx + 1].type ==
                ...
                new_field.array_len =
                    get_array_len(lexer, l_subscr_idx, &field_end_idx,
                                  new_field.name, false, NULL);
                ++field_end_idx;
            }
        }

        if (field_end_idx >= lexer->token_tbl.size ||
            lexer->token_tbl.elems[field_end_idx].type != TokenType_SEMICOLON) {
            ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                         lexer->token_tbl.elems[field_name_idx].file_path,
                         "missing semicolon on line %u.\n",
                         lexer->token_tbl.elems[field_name_idx].line_num);
        ...
        field_size = PrimitiveType_size(new_field.type, new_field.lvls_of_indir,
...
static u32 parse_struct_decl(const struct Lexer *lexer, u32 decl_idx,
                             struct BlockNode *block, u32 *sp, u32 bp,
                             bool *became_func_decl)
{
    ...
    if (l_curly_idx >= lexer->token_tbl.size ||
        (lexer->token_tbl.elems[l_curly_idx].type != TokenType_L_CURLY &&
         lexer->token_tbl.elems[l_curly_idx].type != TokenType_SEMICOLON &&
        ...
        ErrMsg_print(ErrMsg_on, &Parser_error_occurred,
                     ...
                     " column %u.\n",
                     new_struct.name, lexer->token_tbl.elems[decl_idx].line_num,
                     lexer->token_tbl.elems[decl_idx].column_num);
        ...
        if (lexer->token_tbl.elems[ident_idx + 1].type != TokenType_L_PAREN) {
            struct VarDeclNode *var_decl = parse_var_decl(
                lexer, decl_idx, &end_idx, bp, sp, false, NULL, block);
        ...
        return end_idx;
    } else if (lexer->token_tbl.elems[l_curly_idx].type ==
               TokenType_SEMICOLON) {
        ...
        if (new_struct.members.size == 0) {
            WarnMsg_print(WarnMsg_on && CompArgs_args.pedantic,
                          &Parser_error_occurred, new_struct.def_file_path,
        ...
        if (prev_decl_struct_idx == m_u32_max)
            ...
            if (structs.elems[prev_decl_struct_idx].defined) {
                ErrMsg_print(
                    ErrMsg_on, &Parser_error_occurred, new_struct.def_file_path,
                    "struct '%s' redefined on line %u. first defined on"
                    " line %u.\n",
                    new_struct.name, new_struct.def_line_num,
    ...
    *block = BlockNode_init();
    ...
    while (prev_end_idx + 1 < lexer->token_tbl.size) {
        ...
        if (lexer->token_tbl.elems[start_idx].type == TokenType_L_CURLY) {
            struct BlockNode *new_block =
                parse(lexer, parent_func, sp - m_TypeSize_stack_frame_size,
            ...
            check_if_missing_r_curly(lexer, block_start_idx, prev_end_idx, true,
                                     missing_r_curly);
            ...
            prev_end_idx = parse_ret_stmt(lexer, block, bp, start_idx,
                                          parent_func, n_blocks_deep);
        } else if (Ident_type_spec(token_src, &typedefs) != PrimType_INVALID ||
                   Ident_modifier_str_to_tok(token_src) != TokenType_NONE) {
            unsigned ident_idx = TypeSpec_read(
                &lexer->token_tbl, start_idx, NULL, NULL, NULL, NULL, &typedefs,
                &structs, &Parser_error_occurred);
            ...
            if (ident_idx + 1 >= lexer->token_tbl.size ||
                ...
                var_decl = parse_var_decl(lexer, start_idx, &prev_end_idx, bp,
                                          &sp, false, NULL, block);
                ASTNodeList_push_back(
                    &block->nodes,
                    ASTNode_create(lexer->token_tbl.elems[start_idx].line_num,
            ...
            prev_end_idx = parse_if_stmt(lexer, block, n_blocks_deep, start_idx,
                                         bp, sp, parent_func);
        }
        ...
        else if (lexer->token_tbl.elems[start_idx].type == TokenType_FOR_STMT) {
            prev_end_idx = parse_for_stmt(lexer, block, n_blocks_deep,
                                          start_idx, bp, sp, parent_func);
        }

        else if (lexer->token_tbl.elems[start_idx].type == TokenType_TYPEDEF) {
            ...
            prev_end_idx = parse_struct_decl(lexer, start_idx, block, &sp, bp,
                                             &became_func_decl);
            ...
            struct DebugPrintRAX *debug_node = safe_malloc(sizeof(*debug_node));
            ASTNodeList_push_back(
                &block->nodes,
                ASTNode_create(lexer->token_tbl.elems[start_idx].line_num,
                               lexer->token_tbl.elems[start_idx].column_num,
            ...
            struct Expr *expr = parse_expr(lexer, start_idx, &prev_end_idx, bp);
    ...
    block->var_bytes =
        round_up(block->var_bytes, m_TypeSize_stack_min_alignment);
