...
static void fold_expr(struct Expr **expr, const struct StructList *structs)
{
    ...
    **expr =
        Expr_create(old_expr.line_num, old_expr.column_num, old_expr.src_start,
                    old_expr.src_len, old_expr.file_path, NULL, NULL, old_type,
                    old_type, 0, 0, ExprPtrList_init(), value, ArrayLit_init(),
                    0, ExprType_INT_LIT, false, 0);
}
...
void RetNode_const_fold(struct RetNode *self, const struct StructList *structs)
{
    Expr_const_fold(&self->value, structs);
}
...
void ForNode_const_fold(struct ForNode *self, const struct StructList *structs)
{
...
void ASTNode_const_fold(struct ASTNode *self, const struct StructList *structs)
{
    if (self->node_struct) {
