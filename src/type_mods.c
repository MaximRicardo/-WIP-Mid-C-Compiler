...
struct TypeModifiers TypeModifiers_combine(
    const struct TypeModifiers *self, const struct TypeModifiers *other,
    ATTRIBUTE((unused)) bool log_errs, ATTRIBUTE((unused)) bool *logged_errs)
{
    struct TypeModifiers new_mods =
        TypeModifiers_create(self->is_static | other->is_static);
    return new_mods;
}
